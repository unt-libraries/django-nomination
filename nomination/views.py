...
class URLForm(forms.Form):
    ...
    nominator_name = forms.CharField(required=False,
        ...
        'id':'your-name-value',}))
    nominator_email = forms.EmailField(required=False,
        widget=forms.TextInput(attrs={'name': 'email-value',
        'id':'email-value',}))
    nominator_institution = forms.CharField(required=False,
        widget=forms.TextInput(attrs={'name': 'institution-value',
        'id':'institution-value', 'autocomplete': 'off'}))

class ScopeForm(forms.Form):
    scope = forms.ChoiceField(required=False,
...
def url_report(request, slug):
    ...
    #get the list of URLs
    try:
        url_list = URL.objects.filter(attribute__iexact='surt', url_project=project).order_by('value')
    except:
        raise http.Http404
