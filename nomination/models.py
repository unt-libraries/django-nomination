...
#    metadata = models.ForeignKey(Metadata)
    value = models.CharField(max_length=255, help_text='Permitted value for associated metadata field.')
    key = models.CharField(max_length=35, help_text='Up to 35 character identifier for the metadata field.', unique=True)
...
#    project = models.ForeignKey(Project)
    name = models.SlugField(max_length=50, help_text='Assign a name for the metadata field (letters, numbers, underscores, and hyphens are permissible).')
#    required = models.BooleanField(help_text='Are users required to submit data for this field when nominating a URL?')
    values = models.ManyToManyField(Value, through='Metadata_Values', blank=True, help_text='Allowed value for metadata field.', verbose_name='values')
    value_sets = models.ManyToManyField(ValueSet, blank=True, help_text='In addition to values manually assigned, values in selected pre-defined sets will also be available to metadata fields.', verbose_name='metadata value sets')
...
class Metadata_Values(models.Model):
    ...
    value_order = models.PositiveIntegerField(default=1, help_text='Change the ordering of the value fields, ordered lowest to highest')
...
class Project_Metadata(models.Model):
    ...
    metadata = models.ForeignKey(Metadata)
    required = models.BooleanField(help_text='Are users required to submit data for this field when nominating a URL?')
    form_type = models.CharField(max_length=30, choices=FORM_TYPES, help_text='Type of HTML form element that should represent the field.')
    help = models.CharField(max_length=255, blank=True, help_text='String used on Web forms to prompt users for accurate data.')
    description = models.CharField(max_length=255, help_text='Used as a descriptive title for the metadata field on Web forms.')
    metadata_order = models.PositiveIntegerField(default=1, help_text='Change the ordering of the metadata fields, ordered lowest to highest')
...
class Nominator(models.Model):
    ...
    nominator_institution = models.CharField(max_length=100, help_text='Your institutional affiliation.')
    nominator_email = models.CharField(max_length=100, help_text='An email address for identifying your nominations in the system.')
...
class URL(models.Model):
    url_project = models.ForeignKey(Project, help_text='The project for which you want to add a URL.')
    ...
    attribute = models.CharField(max_length=255, help_text='A property of the URL you wish to describe.')
    ...
    def entity_display(self):
        return "<a href=\'http://%s/admin/nomination/url/%s\'>%s</a>&nbsp;<a href=\'%s\'><img src=\'%snomination/images/external-link.gif\'/></a>" \
                   % (Site.objects.get_current().domain, self.id, self.entity, self.entity, settings.STATIC_URL)
