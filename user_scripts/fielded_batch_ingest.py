...
#Displayed message to user if command line entry is wrong or help is entered
def usage():
    """
                ...
                if not attribute_name == 'url' and data[attribute_name] != '':
                    ...
                    if isinstance(data[attribute_name], list):
                        for val in data[attribute_name]:
                            new_entry = create_url_entry(project, nominator, url_entity, attribute_name, val)
                            if new_entry:
                                entry_count += 1
                    else:
                        new_entry = create_url_entry(project, nominator, url_entity, attribute_name, data[attribute_name])
...
def create_url_entry(project, nominator, url_entity, url_attribute, url_value):
    ...
    try:
        if url_attribute == 'surt':
            existing_url = URL.objects.get(url_project=project,
        ...
        try:
            new_url = URL(url_project = project,
                                     url_nominator = nominator,
                                     entity = url_entity,
...
if __name__ == "__main__":
    ...
    if opt_dict.nominator_id != None and opt_dict.project_slug != None:
